...
##### 代码结构如下
```
└── game_name_24            // 代码目录(强制命名方式：游戏名_模板ID)
    ├── assets              // 资源文件目录(强制命名)
    │   ├── cdn             // CDN资源文件目录(强制命名), 存放图片、音频等资源，不允许存放js/css
    │   │   └── xxx.xx
    │   ├── js              // js代码目录(非强制命名)
    │   │   └── xxx.js
    │   │   └── ...
...
```
##### 资源文件访问路径
- CDN资源文件访问路径: `[CDN资源路径]/[游戏名_模板ID]/assets/cdn/xxx.xx`
- 其他资源文件访问路径: `<?php echo Yii::app()->params['src_basic_url'];?>/[模板ID]/assets/...`
...
<script type="text/javascript" src="<?php echo Yii::app()->params['src_basic_url'];?>/js/skip/main.js?v=<?php echo Yii::app()->params['version']; ?>"></script>
```
####DOM示例
说明：DOM类游戏的DOM结构就写在页面文件中，同时引入相应的CSS和JS
```php
<!-- 老虎机 抽奖类 DOM -->
<link rel="stylesheet" href="<?php echo Yii::app()->params['src_basic_url'];?>/css/game/tiger.css?v=<?php echo Yii::app()->params['version']; ?>">
<div id="game_div">
    <div class="title"></div>
...
<?php require_once('main.php'); ?>
<script type="text/javascript" src="<?php echo Yii::app()->params['src_basic_url'];?>/js/tiger/main.js?v=<?php echo Yii::app()->params['version']; ?>"></script>
```
